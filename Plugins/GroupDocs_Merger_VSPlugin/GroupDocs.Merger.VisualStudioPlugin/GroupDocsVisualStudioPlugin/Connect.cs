...
using GroupDocsMergerVisualStudioPlugin.GUI;
using GroupDocsMergerVisualStudioPlugin.Properties;

namespace GroupDocsMergerVisualStudioPlugin
{
    public class Connect : Extensibility.IDTExtensibility2, IDTCommandTarget
    {
        // Constants for command properties
        private const string GROUPDOCS_MENU_NAME = "MenuGroupDocsVSPlugin";
        private const string GROUPDOCS_MENU_CAPTION = "GroupDocs";
        private const string GROUPDOCS_MENU_TOOLTIP = "Download and Run GroupDocs.Merger for .NET API Examples";

        private const string Merger_COMMAND_NAME = "GroupDocsMergerVSPlugin";
        private const string Merger_COMMAND_CAPTION = "New GroupDocs.Merger Example Project";
        private const string Merger_COMMAND_TOOLTIP = "Download and Run GroupDocs.Merger for .NET API Examples";
            ...
            try
            {
                ...
                for (int iloop = 1; iloop <= toolsPopup.CommandBar.Controls.Count; iloop++)
                {
                    if (toolsPopup.CommandBar.Controls[iloop].Caption == GROUPDOCS_MENU_CAPTION)
                    {
                        ...
                        {
                            if (cmdbtn.Caption == Merger_COMMAND_CAPTION)
                            {
                                oBtn = cmdbtn;
                            }
                        }
                ...
                if (oBtn == null)
                {
                    ...
                    Command myCommand = commands.AddNamedCommand2(addInInstance, Merger_COMMAND_NAME,
                        Merger_COMMAND_CAPTION, Merger_COMMAND_TOOLTIP, false, Resources.pnglogosmall, ref contextGUIDS,
           ...
           ref object varOut, ref bool handled)
        {
            try
            {
                ...
                {
                    if (cmdName == addInInstance.ProgID + "." + Merger_COMMAND_NAME)
                    {
